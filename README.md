...
- Window functions: RANK() OVER (PARTITION BY) to rank
  medication burden within age groups, NTILE(4) to build risk quartiles
- CASE-based tiering to convert continuous variables into
  business-readable risk categories
- Correlated subqueries with HAVING to isolate diagnosis categories
  performing above the population-wide average
- Multi-table joins against ID-mapping reference tables to convert
  numeric codes into readable labels
